...
- With the paths matching, create, rename, move, and delete vault files with `obsidian create`, `obsidian rename`, `obsidian move`, and `obsidian delete`, which rewrite the wikilinks pointing at a renamed or moved note. Using git instead leaves every inbound link dangling behind a diff that looks clean, so `rg` the old filename, confirm zero hits, and update each one. To target a vault explicitly, put `vault=<name>` before the command; placed after it, it is ignored rather than refused.
